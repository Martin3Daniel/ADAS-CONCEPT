...
export default function PartnerDiscovery() {
  ...
  const handleDragEnd = (_: MouseEvent | TouchEvent, info: { offset: { x: number } }) => {
    ...
    if (Math.abs(info.offset.x) > dragTolerance) {
      if (info.offset.x < 0 && activeIndex < clients.length - 1) {
    ...
    <section id="partners" className="py-24 bg-gradient-to-br from-gray-50 to-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative" ref={containerRef}>
            ...
            >
              <button
                ...
                className={`p-3 rounded-full transition-all duration-300 ${
                  activeIndex > 0 
                    ? 'bg-white/80 backdrop-blur-sm shadow-lg hover:bg-white border border-gray-200' 
              ...
              <button
                onClick={() => activeIndex < clients.length - 1 && goToSlide(activeIndex + 1)}
                className={`p-3 rounded-full transition-all duration-300 ${
            ...
            >
              {clients.map((client, index) => (
                ...
                >
                  ...
                  <motion.h3
                    className="text-xl sm:text-2xl font-bold text-[#1A1A1A] mb-3 text-center"
                    initial={{ opacity: 0, y: 20 }}
                  ...
                  <motion.p
                    className="text-gray-500 text-center max-w-md mx-auto mb-6"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                  >
                    {isRevealed ? 'Hide Summary' : 'Reveal More'}
                  </motion.button>
          ...
          {!isDragging && (
            <motion.div
              className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 text-gray-400 text-xs"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ delay: 1 }}
            >
              ...
              <span>Drag to explore partners</span>
            </motion.div>
          )}
        </div>
      </div>
